...
const char KWH_TYPE[] = "ACE9000-BALI";
...
bool SettingsInit()
{
  ...
  if (strlen(settings.getChar("kwh.merk")) == 0 or strlen(settings.getChar("kwh.merk")) == 0)
  {
...
void TokenProcess(/*char *token*/)
{
  if (received_msg and (strcmp(received_topic, topicREQToken) == 0) and received_length == 20 and status == Status::IDLE)
  {
    ...
    // MqttPublishStatus("RECEIVE TOKEN" + String(received_payload));
#ifdef USE_LOG
...
{
  File logFile;
  if (SPIFFS.exists(logFileNowName))
  {
  }
  else
    logFile = SPIFFS.open(logFileNowName, "w");
  if (logFile.size() > 0)
  {
