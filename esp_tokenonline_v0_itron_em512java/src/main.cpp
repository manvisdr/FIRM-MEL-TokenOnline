...
const char KWH_MERK[] = "ITRON";
const char KWH_TYPE[] = "EM512-JAVA";
...
void wifi_connect_cb()
{
  // wifi_connect_task.disable();
  if (!dhcpbuff)
    WiFi.config(IPaddress, IPgateway, IPsubnet, IPdns1, IPdns2);
...
bool SoundDetectShortItron()
{
  ...
  if (detectedShortItronFast and (detectShortCnt > 10 /*25 and detectShortCnt < 35)*/))
  {
...
void SoundDetectValidasiV2()
{
  ...
  data_peak_prev = data_peak_now;
  data_peak_now = data_peak_last;
  data_peak_last = SoundPeak;
  data_peak_rata = (data_peak_prev + data_peak_now + data_peak_last) / 3;

  bolprevValidSound = bolnowValidSound;
  bolnowValidSound = bolnexValidSound;
  bolnexValidSound = data_peak_rata > 50;
  ...
  Serial.print(data_peak_prev);
  Serial.print(" ");
  Serial.print(data_peak_now);
  Serial.print(" ");
  Serial.print(data_peak_last);
  Serial.print(" ");
  Serial.print((data_peak_prev + data_peak_now + data_peak_last) / 3);
  Serial.print(" ");
...
String OutputSoundValidasiV2()
{
  ...
  if (kwhstatusbeepbuff == 1)
  {
    ...
    //   out = "success";
    // }
    if ((valueInterval >= 15 and counterBeep >= 3) or counterBeep >= 3)
    {
      Serial.println("DETECK GAGAL");
      // MqttCustomPublish(topicRSPSoundStatus, "VALIDASI SUARA GAGAL");
      out = "failed";
...
void TokenProcess(/*char *token*/)
{
  ...
  if (status == Status::PROCCESS_TOKEN and
      millis() > (afterSolenoid + kwhtimestatbuff) + (kwhtimenomibuff + 3000) and TokenStateSaldo)
  {
...
void GetLogFile(/*char *token*/)
{
  if (received_msg and strcmp(received_topic, topicREQLogFile) == 0 and strcmp(received_payload, "now") == 0)
  {
    ...
    log_i("[%s:%d] Receive Mqtt: Get Log", __FUNCTION__, __LINE__);
