...
const char KWH_MERK[] = "SMARTMETER";
const char KWH_TYPE[] = "SMI-810";
...
char *timeStr()
{
  ...
  suseconds_t usec;
  timeToCharLog(tm, usec, &timeStr2[0]);
...
// StreamLogger serialLog((Stream *)&Serial, &timeStr, &LOG_PREFIX_SPIFFS[0], ALL);
// // SPIFFS logger
// SPIFFSLogger spiffsLog(SPIFFS_AUTO_FLUSH, logFileNowName, &timeStr, &LOG_PREFIX_SERIAL[0], INFO);
...
void dumpSysLog(char *filename)
{
  int seekVal = 0, len = 0;
  char cIn;

  File sl = fileSys.open(filename, "r");
...
bool DetectionLowToken()
{
  ...
  if (counter > 15)
  {
    // MqttPublishGetAlarmResponse("", "Detected Alarm");
    AlarmStatus = true;
...
void TokenProcess(/*char *token*/)
{
  ...
  {
    if (millis() > (afterSolenoid + kwhtimestatbuff) - 70 and TokenStateStatus)
    {
      camera_fb_t *fbs;
