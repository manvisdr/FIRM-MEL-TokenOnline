...
const char KWH_MERK[] = "MELCOINDA";
const char KWH_TYPE[] = "MTS-125";
...
int kwhalarmbuff;
bool dhcpbuff;
char chripaddressbuff[15];
char chripsubnetbuff[15];
char chripgatewaybuff[15];
// SETTING BUFF
IPAddress IPaddress;
IPAddress IPgateway;
IPAddress IPsubnet;
IPAddress IPdns1(8, 8, 8, 8);
IPAddress IPdns2(8, 4, 8, 4);
...
char topicRespTopup[50];
char topicRespKalibrasi[50];
char topicRespGetSaldo[50];
char topicRespAlarm[50];
...
bool SettingsInit()
{
  ...
  if (strlen(settings.getChar("kwh.merk")) == 0)
  {
    settings.setChar("kwh.merk", KWH_MERK, true);
    settings.setChar("kwh.type", KWH_TYPE, true);
    settings.writeSettings(configFile);
  }

  if (strlen(settings.getChar("ip.addres")) == 0)
  {
    // dhcp":false,"addres":"0.0.0.0","subnet":"0.0.0.0","gateway":"0.0.0.0"}}
    settings.setBool("ip.dhcp", true, true);
    settings.setChar("ip.addres", "0.0.0.0", true);
    settings.setChar("ip.subnet", "0.0.0.0", true);
    settings.setChar("ip.gateway", "0.0.0.0", true);
    settings.writeSettings(configFile);
  }
  ...
  dhcpbuff = settings.getBool("ip.dhcp");
  strcpy(chripaddressbuff, settings.getChar("ip.addres"));
  strcpy(chripsubnetbuff, settings.getChar("ip.subnet"));
  strcpy(chripgatewaybuff, settings.getChar("ip.gateway"));
  ...
  Serial.printf("KWH MEREK: %s\n", settings.getChar("kwh.merk"));
  Serial.printf("KWH TYPE: %s\n", settings.getChar("kwh.type"));

  Serial.printf("IP DHCP STATUS: %d\n", dhcpbuff);
  Serial.printf("IP ADDRESS: %s\n", chripaddressbuff);
  Serial.printf("IP SUBNET: %s\n", chripsubnetbuff);
  Serial.printf("IP GATEWAY: %s\n", chripgatewaybuff);

  IPaddress.fromString(chripaddressbuff);
  IPsubnet.fromString(chripsubnetbuff);
  IPgateway.fromString(chripgatewaybuff);
  Serial.println("**SETTING INIT COMPLETE**");

  Serial.println("**NETWORK CONFIG**");
  Serial.printf("IP DHCP STATUS: %d\n", dhcpbuff);
  Serial.printf("IP ADDRESS: %u.%u.%u.%u\n", IPaddress[0], IPaddress[1], IPaddress[2], IPaddress[3]);
  Serial.printf("IP SUBNET: %u.%u.%u.%u\n", IPsubnet[0], IPsubnet[1], IPsubnet[2], IPsubnet[3]);
  Serial.printf("IP GATEWAY: %u.%u.%u.%u\n", IPgateway[0], IPgateway[1], IPgateway[2], IPgateway[3]);
  Serial.println("**NETWORK CONFIG**");
  return true;
}
...
bool SettingsSaveAllIP(char *ssid, char *pass, int time1, int time2, int freq, long alarm, int statusbeep, bool dhcp, char *ipaddres, char *ipsubnet, char *ipgateway)
{
  int returnVal;
  settings.setChar("wifi.ssid", ssid);
  settings.setChar("wifi.pass", pass);
  ...
  settings.setLong("kwh.alarm", alarm);
  settings.setInt("kwh.statusbeep", statusbeep);
  settings.setBool("ip.dhcp", dhcp);
  settings.setChar("ip.addres", ipaddres);
  settings.setChar("ip.subnet", ipsubnet);
  settings.setChar("ip.gateway", ipgateway);
  ...
  if (event == ESP_SPP_DATA_IND_EVT and KoneksiStage == BT_INIT)
  {
    ...
    if (btReceive.length() > 10)
    {
      ...
      else if (N > 8)
      {
        strcpy(ssidBuffer, sPtr[0]);
        strcpy(passBuffer, sPtr[1]);
        Serial.printf("DATA BT: ssid=%s pass=%s kwhstat=%s kwhnomi=%s kwhstatusbeep=%s kwhfreq=%s kwhalarm=%s ipdhcp=%s ipaddress=%s ipsubnet=%s ipgateway=%s\n",
                      sPtr[0], sPtr[1], sPtr[2], sPtr[3], sPtr[5], sPtr[6], sPtr[7], sPtr[8], sPtr[9], sPtr[10], sPtr[11]);
        kwhstat = atoi(sPtr[2]);
        kwhnomi = atoi(sPtr[3]);
        kwhfreq = atoi(sPtr[6]);
        kwhstatusbeep = atoi(sPtr[5]);
        kwhalarm = atoi(sPtr[7]) * 60000;
        btdhcp = atoi(sPtr[8]);
        Serial.println(btdhcp);
        strcpy(btipaddres, sPtr[9]);
        Serial.println(btipaddres);
        strcpy(btipsubnet, sPtr[10]);
        Serial.println(btipsubnet);
        strcpy(btipgatway, sPtr[11]);
        Serial.println(btipgatway);
        Serial.println("IF ELSE WIFI DHCP DLL");
        if (btdhcp != dhcpbuff)
        {
          Serial.println("Changing dhcp conf--> RESTART");
          SettingsSaveAllIP(ssidBuffer, passBuffer, kwhstat, kwhnomi, kwhfreq, kwhalarm,
                            kwhstatusbeep, btdhcp, btipaddres, btipsubnet, btipgatway);
          SettingsInit();
          ESP.restart();
        }
        else
    ...
    {
      sprintf(respBTBuffer, "ssid=%s pass=%s kwhstat=%d kwhnomi=%d kwhstatusbeep=%d kwhfreq=%d kwhalarm=%d ipdhcp=%d ipaddress=%s ipsubnet=%s ipgateway=%s",
              wifissidbuff, wifipassbuff, kwhtimestatbuff, kwhtimenomibuff, kwhstatusbeepbuff, kwhfreqbuff, kwhalarmbuff, dhcpbuff, chripaddressbuff, chripsubnetbuff, chripgatewaybuff);
      Serial.print("SEND EXISTING DATA -> ");
      Serial.println(respBTBuffer);
      SerialBT.println(respBTBuffer);
      SerialBT.flush();
      memset(respBTBuffer, '\0', sizeof(respBTBuffer));
      // free(respBTBuffer);
    }
  }
}
...
void WifiInit()
{
  ...
  IPAddress gat(192, 168, 100, 1);
  IPAddress sub(255, 255, 255, 255);
  IPAddress dns(8, 8, 8, 8);
  IPAddress dns2(8, 4, 8, 4);

  Serial.println("**WIFI INIT START**");
  // WiFi.config(ip, gat, sub, dns, dns2);
  wifi_connect_cb();
...
String sendPhotoViewKWH(char *sn)
{
  ...
  Serial.println("Uploading Image Balance");
  ...
  if (postClient.connect(serverName.c_str(), serverPort))
  {
    ...
    while (tOut > millis())
    {
      ...
      {
        getBody = postClient.readStringUntil('\r');
        return (getBody);
      }
    }
    // postClient.stop();
...
void MqttPublishTopupResponse(char *subTopic, String msg)
{
  char fulltopic[70];
  strcpy(fulltopic, topicRespTopup);
  strcat(fulltopic, subTopic);
  MqttCustomPublish(fulltopic, msg);
}

void MqttPublishGetSaldoResponse(char *subTopic, String msg)
{
  char fulltopic[70];
  strcpy(fulltopic, topicRespGetSaldo);
  strcat(fulltopic, subTopic);
  MqttCustomPublish(fulltopic, msg);
}

void MqttPublishGetKalibrasiResponse(char *subTopic, String msg)
{
  char fulltopic[70];
  strcpy(fulltopic, topicRespKalibrasi);
  strcat(fulltopic, subTopic);
  MqttCustomPublish(fulltopic, msg);
}

void MqttPublishGetAlarmResponse(char *subTopic, String msg)
{
  char fulltopic[70];
  strcpy(fulltopic, topicRespAlarm);
  strcat(fulltopic, subTopic);
  MqttCustomPublish(fulltopic, msg);
}
...
void MqttTopicInit()
{
  ...
  sprintf(topicRespAlarm, "%s/%s/%s/%s", mqttChannel, "respond", idDevice, "event/alarm");
  sprintf(topicRespKalibrasi, "%s/%s/%s/%s", mqttChannel, "respond", idDevice, "event/kalibrasi");
  sprintf(topicRespTopup, "%s/%s/%s/%s", mqttChannel, "respond", idDevice, "event/topup");
  sprintf(topicRespGetSaldo, "%s/%s/%s/%s", mqttChannel, "respond", idDevice, "event/balance");
}
...
void MqttReconnect()
{
  ...
  while (!mqtt.connected() and WiFi.status() == WL_CONNECTED)
  {
    ...
    if (mqtt.connect(clientId.c_str()))
    {
      ...
      // log_i("[%s:%d] Mqtt Reconnected", __FUNCTION__, __LINE__);
...
bool DetectionLowToken()
{
  ...
  if (AlarmStatus and timeAlarm)
  {
    ...
    CaptureAndSendNotif();
    // log_i("[%s:%d] Detection Alarm Low Balance", __FUNCTION__, __LINE__);

    MqttPublishGetAlarmResponse("", "Send Notification");
    // log_i("[%s:%d] Send Notification Low Balance", __FUNCTION__, __LINE__);
    counter = 0;
    AlarmStatus = false;
    prevAlarmStatus = false;
    timeAlarm = false;
...
int DetectLowTokenV2()
{
  ...
  bolnexAlarmSound = SoundPeak == kwhfreqbuff || (SoundPeak == kwhfreqbuff + 1 || SoundPeak == kwhfreqbuff - 1) || (melcoinda > 5 and melcoinda < 5);
...
void SoundDetectValidasiV2()
{
  ...
  // MELCOINDA
  bolnexValidSound = SoundPeak == melcoinda || (SoundPeak == melcoinda + 1 || SoundPeak == melcoinda - 1) || (SoundPeak > melcoinda - 20 and SoundPeak < melcoinda - 20);
  // MELCOINDA
...
String OutputSoundValidasiV2()
{
  ...
  if (kwhstatusbeepbuff == 1)
  {
    ...
    // MELCOINDA
    if ((valueInterval >= 1 and counterBeep >= 2) or counterBeep >= 1)
    {
      Serial.println("DETECK BENAR");
      // MqttCustomPublish(topicRSPSoundStatus, "VALIDASI SUARA BENAR");
      out = "success";
...
void GetKwhProcess()
{
  if (received_msg and strcmp(received_topic, topicREQView) == 0 /*and strcmp(received_payload, "view") == 0*/)
  {
    ...
    Serial.println("---------------GET_BALANCE------------------");
    MqttPublishResponse("view");
    MqttPublishGetSaldoResponse("", "View KWH Balance");
    // if (stateBt)
    // {
    btStop();
    // log_w("[%s:%d] Bluetooth Stop if Upload from SPIFFS", __FUNCTION__, __LINE__);
    //   stateBt = false;
    // }
    // log_i("[%s:%d] Receive Mqtt: Get KWH", __FUNCTION__, __LINE__);
    ...
    // log_i("[%s:%d] Capture And Upload Image Get KWH : success", __FUNCTION__, __LINE__);
    Serial.println("---------------GET_BALANCE------------------");
    received_msg = false;
    status = Status::IDLE;
  }
...
{
  if (sendImageNominal(nominalFileSpiffs, tokenString) == "HTTP/1.1 200 OK")
  {
  ...
  {
    if (sendImageSaldo(saldoFileSpiffs, tokenString) == "HTTP/1.1 200 OK")
    {
  ...
  if (flagUploadSaldo)
  {
    if (sendImageStatusToken(statusFileSpiffs, StrSoundValidasi, tokenString, idDevice) == "HTTP/1.1 200 OK")
    {
...
void TokenProcess(/*char *token*/)
{
  ...
  {
    status = Status::PROCCESS_TOKEN;
    Serial.println("---------------TOKEN_PROCESS------------------");
    MqttPublishResponse("topup");
    strcpy(tokenString, received_payload);
    MqttPublishTopupResponse("/event", "Topup Process Start");
    MqttPublishTopupResponse("/token", tokenString);
    // if (stateBt)
    // {
    //   btStop();
    //   // log_w("[%s:%d] Bluetooth Stop if Upload from SPIFFS", __FUNCTION__, __LINE__);
    ...
    // }
    btStop();
    // log_i("[%s:%d] Receive Mqtt: Topup Process", __FUNCTION__, __LINE__);
    // log_i("[%s:%d] Topup Process -> Token: %s", __FUNCTION__, __LINE__, tokenString);
  ...
  if (status == Status::PROCCESS_TOKEN and millis() < afterSolenoid + kwhtimestatbuff)
  {
    ...
    // MELCOINDA
    if (millis() > afterSolenoid + (kwhtimestatbuff / 3) and
        ...
        millis() < afterSolenoid + kwhtimestatbuff and TokenStateStatus)
    {
      ...
      else
      {
        ...
        // log_i("[%s:%d] Token Process: Capture Status Success", __FUNCTION__, __LINE__);
      ...
      TokenStateNominal)
  {
    ...
    if (TokenStateNominal)
    {
      ...
      else
      {
        ...
        // log_i("[%s:%d] Token Process: Capture Nominal Success", __FUNCTION__, __LINE__);
      ...
      millis() > (afterSolenoid + kwhtimestatbuff) + (kwhtimenomibuff + 2000) and TokenStateSaldo)
  {
    ...
    if (TokenStateSaldo)
    {
      ...
      else
      {
        ...
        // log_i("[%s:%d] Token Process: Capture Saldo Success", __FUNCTION__, __LINE__);
    ...
    // log_i("[%s:%d] Token Process -> Sound Validation: %s", __FUNCTION__, __LINE__, StrSoundValidasi);
    status = Status::IDLE;
  }
...
String sendImageKalibrasi(char *part, uint8_t *data_pic, size_t size_pic)
{
  ...
  Serial.println("Upload Image Kalibrasi...");
  if (postClient.connect(SERVER.c_str(), PORT))
  {
    Serial.println("...Connected Server");
...
void RequestKalibrasiSolenoids()
{
  ...
  {
    status = Status::WORKING;
    Serial.println("---------------GET_KALIBRASI------------------");
    MqttPublishResponse("calibrate");
    MqttPublishGetKalibrasiResponse("", "Kalibrasi 1 Start");
    // if (stateBt)
    // {
    //   btStop();
    //   // log_w("[%s:%d] Bluetooth Stop if Upload from SPIFFS", __FUNCTION__, __LINE__);
    ...
    // }
    btStop();
    // log_i("[%s:%d] Receive Mqtt: Calibrate Process", __FUNCTION__, __LINE__);
    ...
    else
    {
      ...
      Serial.println(res);
      // log_i("[%s:%d] Capture And Upload Image Calibrate 1 : success", __FUNCTION__, __LINE__);
      esp_camera_fb_return(fb);
    }

    MqttPublishGetKalibrasiResponse("", "Kalibrasi 1 Done");
    MqttPublishGetKalibrasiResponse("", "Kalibrasi 2 Start");
#ifdef USE_SOLENOID
    MechanicTyping(5);
#endif
    delay(delaySolenoidCalib);
    ...
    else
    {
      ...
      // log_i("[%s:%d] Capture And Upload Image Calibrate 2 : success", __FUNCTION__, __LINE__);
    ...
    MqttPublishGetKalibrasiResponse("", "Kalibrasi 2 Done");
...
String LogUpload(char *filename, char *tgl)
{
  ...
  if (postClient.connect(SERVER.c_str(), PORT))
  {
    ...
    while (logFile.available() /*and !millis() > timenow*/)
    {
      size_t n = logFile.readBytes((char *)buffer, bufferSize);
      // Serial.println(millis());
      if (millis() > timenow)
      {
        flagTimeout = true;
...
// void SystemLoggerInit()
// {
//   // Logging::addAppender(new FSAppender(SPIFFS, "/mylog", 1));
//   Logging::addBufferedAppender(new FSAppender(SPIFFS, "/mylog", 1), 10, false);
...
void CheckingLog()
{
  // //log_i("Check File Log");
  File logFile;

  logFile = SPIFFS.open(logFileNowName, "r");
  if (logFile.size() > 0)
  {
    logFile.close();
    Serial.println("file berisi");
    RemovingFile(logFileNowName);
    CreatingFile(logFileNowName);
  }
}
...
void setup()
{
  ...
  // Alarm.alarmRepeat(0, 30, 0, CheckingLog); // 8:30am every day
  pinMode(4, OUTPUT);
  pinMode(33, OUTPUT);
  digitalWrite(33, LOW);
  status = Status::IDLE;
...
void loop()
{
  ...
  if (WiFi.status() == WL_CONNECTED)
  {
    if (newToken and mqtt.connected())
    {
      ...
      MqttPublishTopupResponse("/event", "Topup Process Done");
      // log_i("[%s:%d] Token Process: Upload All Image Success", __FUNCTION__, __LINE__);
      // log_i("[%s:%d] Token Process Done", __FUNCTION__, __LINE__);
      Serial.println("---------------TOKEN_PROCESS------------------");
    }
  }
}
