...
const char KWH_MERK[] = "HEXING";
const char KWH_TYPE[] = "HXE116-KP";
...
long timeValidasi_begin;
long timeValidasi_last;
long timeValidasi_val;
...
String sendPhotoViewKWH(char *sn)
{
  ...
  Serial.println("Upload Image GetKWH");
...
int DetectLowTokenV2()
{
  ...
  if (bolprevAlarmSound < bolnowAlarmSound and bolnowAlarmSound > bolnexAlarmSound)
  {
    ...
    prevAlarmStatus = true;
    counterAlarm++;
    Serial.print("DETECTED BEEP ALARM");
    Serial.print("");
...
void SoundDetectValidasiV2()
{
  ...
  // bolnexValidSound = data_peak_rata > 65;
  bolnexValidSound = SoundPeak == kwhfreqbuff || (SoundPeak == kwhfreqbuff + 1 || SoundPeak == kwhfreqbuff - 1) || (SoundPeak > kwhfreqbuff - 20 and SoundPeak < kwhfreqbuff - 20);

  if (bolprevValidSound < bolnowValidSound)
  {
    beginCnt = counterValidasiSound;
    timeValidasi_begin = millis();
  ...
  else if (bolnowValidSound > bolnexValidSound)
  {
    ...
    timeValidasi_last = millis() - timeValidasi_begin;
    timeValidasi_val += timeValidasi_last;
  }
  ...
  Serial.println(timeValidasi_val);
}
...
String OutputSoundValidasiV2()
{
  ...
  if (kwhstatusbeepbuff == 1)
  {
    if (timeValidasi_val > 540)
    {
      Serial.println("DETECK GAGAL");
      // MqttCustomPublish(topicRSPSoundStatus, "VALIDASI SUARA GAGAL");
      out = "failed";
    }
    else if (timeValidasi_val == 0)
    {
      Serial.println("DETECK INVALID");
      // MqttCustomPublish(topicRSPSoundStatus, "VALIDASI SUARA INVALID");
      out = "invalid";
...
String sendImageKalibrasiV2(char *part, uint8_t *data_pic, size_t size_pic)
{
  String serverRes;
  WiFiClient postClient;
  String bodyKalibrasi = body_Kalibrasi();
...
void RequestKalibrasiSolenoids()
{
  if (received_msg and strcmp(received_topic, topicREQKalibrasi) == 0 /* and strcmp(received_payload, "calibrate") == 0 */)
  {
    ...
    else
    {
      ...
      res = sendImageKalibrasiV2("1", fb->buf, fb->len);
#ifdef USE_LOG
    ...
    else
    {
      ...
      res = sendImageKalibrasiV2("2", fb->buf, fb->len);
#ifdef USE_LOG
